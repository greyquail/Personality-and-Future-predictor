...
# -----------------------------------------------------------------------------
# Ensure you have set your API key in the Streamlit Cloud Secrets (no extra spaces or angle brackets)
...
def predict_multiverse(choice, mindset_score):
    ...
    For a positive sentiment, we sample 4 outcomes from our base list.
    For a negative sentiment, we sample 2 outcomes.
    """
    base_futures = [
    ...
    if mindset_score > 0:
        # For positive sentiment, sample 4 outcomes.
        possible_futures = random.sample(base_futures, 4)
    else:
        # For negative sentiment, sample 2 outcomes.
...
if st.button("Predict My Future"):
    if not user_choice or not mindset_text:
        ...
        for idx, (fut, prob) in enumerate(futures, start=1):
            # Generate explanation for each future using OpenAI GPT

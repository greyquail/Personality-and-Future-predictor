...
def load_model():
    model_name = "EleutherAI/gpt-neo-125M"  # Smaller model for faster loading and lower resource usage
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    model = AutoModelForCausalLM.from_pretrained(
        model_name,
        torch_dtype=torch.float32,  # Using float32 on CPU for stability
